...
# 3. 初始化本地 D1 数据库 (重要: 每次 schema.sql 变更后都需要重新初始化)
#    此命令会删除并重建本地 D1 数据库，并应用最新的 schema.sql
npm run init-db
...
3. **初始化数据库**
   - **重要：** 在 D1 数据库中执行 `schema.sql`。你可以使用 Cloudflare Dashboard 的 D1 界面，或者通过 `wrangler d1 execute <YOUR_D1_DATABASE_NAME> --file=schema.sql` 命令来执行。
...
5. **初始化数据库**
   ```bash
   # 重要：此命令会删除并重建本地 D1 数据库，并应用最新的 schema.sql
...
### 数据库操作
```bash
# 初始化本地 D1 数据库 (重要: 每次 schema.sql 变更后都需要重新初始化)
# 此命令会删除并重建本地 D1 数据库，并应用最新的 schema.sql
npm run init-db

# 生产数据库迁移 (请替换 <YOUR_D1_DATABASE_NAME> 为你的实际数据库名称)
# wrangler d1 execute <YOUR_D1_DATABASE_NAME> --file=schema.sql
```
