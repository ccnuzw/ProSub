  ...
  <a-card>
    <div style="margin-bottom: 16px; display: flex; justify-content: space-between; align-items: center">
      ...
      <a-space>
        ...
        <a-button type="default" :icon="$slots.importOutlined ? $slots.importOutlined() : ''" @click="setIsImportModalVisible(true)">导入节点</a-button>
    ...
    <a-modal title="从剪贴板导入节点" v-model:open="isImportModalVisible" @ok="handleImport" @cancel="() => setIsImportModalVisible(false)" :confirm-loading="importing" ok-text="导入" cancel-text="取消">
      <p>请粘贴一个或多个节点链接，每行一个。</p>
      <a-textarea :rows="10" v-model:value="importLinks" placeholder="vmess://...&#10;vless://...&#10;ss://..."/>
    </a-modal>
  </a-card>
</template>
...
const handleImport = async () => {
  ...
  try {
    const res = await fetch('/api/nodes/batch-import', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ links: importLinks.value }),
    })
