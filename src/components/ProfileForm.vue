<template>
  <a-form :model="formState" layout="vertical" @finish="onFinish">
    <a-form-item name="name" label="配置文件名称" :rules="[{ required: true, message: '请输入配置文件名称' }]" >
    ...
    <div style="display: flex; align-items: center; gap: 16px">
      <div style="flex: 1; min-width: 0">
        ...
        >
          <template #extra>
            <a-input-search placeholder="搜索..." v-model:value="nodeSearchTerm" style="width: 200px" />
          </template>
          <a-list style="height: 400px; overflow: auto" :data-source="availableNodes">
            <template #renderItem="{ item: node }">
              <a-list-item>
                <a-checkbox :checked="checkedNodeIds.includes(node.id)" @change="(e) => handleNodeCheckChange(node.id, e.target.checked)">
                  <a-list-item-meta>
                    <template #avatar><ClusterOutlined /></template>
                    <template #title><a-typography-text style="display: inline-block; max-width: 250px" ellipsis={{ tooltip: node.name }}>{{ node.name }}</a-typography-text></template>
                    <template #description>
                      <a-space>
                        <a-tag :color="getNodeStatusColor(node)">{{ getNodeStatusText(node) }}</a-tag>
                ...
                <template #actions>
                  <a-button shape="circle" size="small" :icon="selectedNodeIds.includes(node.id) ? h(ArrowLeftOutlined) : h(ArrowRightOutlined)" @click="() => toggleNodeSelection(node.id)" />
                </template>
              </a-list-item>
            </template>
            <template #emptyText>
      ...
      <a-space direction="vertical">
        <a-button :icon="h(ArrowRightOutlined)" @click="moveCheckedNodes" :disabled="checkedNodeIds.filter(id => !selectedNodeIds.includes(id)).length === 0" />
        <a-button :icon="h(ArrowLeftOutlined)" @click="removeCheckedNodes" :disabled="checkedNodeIds.filter(id => selectedNodeIds.includes(id)).length === 0" />
      </a-space>
      <div style="flex: 1; min-width: 0">
    ...
    <div style="display: flex; align-items: center; gap: 16px">
              ...
              <a-list-item>
                <a-checkbox :checked="checkedSubIds.includes(sub.id)" @change="(e) => handleSubCheckChange(sub.id, e.target.checked)">
                  <a-list-item-meta>
                    ...
                    <template #title><a-typography-text style="display: inline-block; max-width: 250px" ellipsis={{ tooltip: sub.name }}>{{ sub.name }}</a-typography-text></template>
                    <template #description><a-typography-text style="display: inline-block; max-width: 250px" ellipsis={{ tooltip: sub.url }}>{{ sub.url }}</a-typography-text></template>
                  </a-list-item-meta>
                </a-checkbox>
                <template #actions>
                  <a-button shape="circle" size="small" :icon="selectedSubIds.includes(sub.id) ? h(ArrowLeftOutlined) : h(ArrowRightOutlined)" @click="() => toggleSubSelection(sub.id)" />
                </template>
              </a-list-item>
            </template>
            <template #emptyText>
      ...
      <a-space direction="vertical">
        <a-button :icon="h(ArrowRightOutlined)" @click="moveCheckedSubs" :disabled="checkedSubIds.filter(id => !selectedSubIds.includes(id)).length === 0" />
        <a-button :icon="h(ArrowLeftOutlined)" @click="removeCheckedSubs" :disabled="checkedSubIds.filter(id => selectedSubIds.includes(id)).length === 0" />
      </a-space>
      <div style="flex: 1; min-width: 0">
...
const selectedNodeIds = ref<string[]>(props.profile?.nodes || [])
const selectedSubIds = ref<string[]>(props.profile?.subscriptions || [])
...
watch(() => props.profile, (newProfile) => {
  if (newProfile) {
    Object.assign(formState, newProfile)
    selectedNodeIds.value = newProfile.nodes || []
    selectedSubIds.value = newProfile.subscriptions || []
  }
})
...
const onFinish = async (values: ProfileFormValues) => {
  ...
  const url = props.profile ? `/api/profiles/${props.profile.id}` : '/api/profiles'
  const dataToSend = { name: values.name, alias: values.alias, nodes: selectedNodeIds.value, subscriptions: selectedSubIds.value }
...
const availableNodes = computed(() => {
  ...
  const available = allNodes.value
...
const moveCheckedNodes = () => {
  const toMove = checkedNodeIds.value.filter(id => !selectedNodeIds.value.includes(id))
  selectedNodeIds.value = [...selectedNodeIds.value, ...toMove]
  checkedNodeIds.value = []
}

const removeCheckedNodes = () => {
  const toKeep = selectedNodeIds.value.filter(id => !checkedNodeIds.value.includes(id))
  selectedNodeIds.value = toKeep
  checkedNodeIds.value = []
}

const moveCheckedSubs = () => {
  const toMove = checkedSubIds.value.filter(id => !selectedSubIds.value.includes(id))
  selectedSubIds.value = [...selectedSubIds.value, ...toMove]
  checkedSubIds.value = []
}

const removeCheckedSubs = () => {
  const toKeep = selectedSubIds.value.filter(id => !checkedSubIds.value.includes(id))
